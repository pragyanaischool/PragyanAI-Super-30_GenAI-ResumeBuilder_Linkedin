...
class PDF(FPDF):
    ...
    def add_experience(self, exp_list):
        for exp in exp_list:
            ...
            self.multi_cell(0, 5, f"{exp.get('title', '')} at {exp.get('company', '')}")
            self.set_font(FONT_FAMILY, 'I', 10)
            self.multi_cell(0, 5, f"{exp.get('duration', '')}")
            self.set_font(FONT_FAMILY, '', 11)
            # Handle bullet points in description
            for point in exp.get('description', '').split('• '):
                if point.strip():
                    self.multi_cell(0, 5, f"• {point.strip()}")
            self.ln(3)

    def add_education(self, edu_list):
        for edu in edu_list:
            self.set_font(FONT_FAMILY, 'B', 11)
            self.multi_cell(0, 5, edu.get('institution', ''))
            self.set_font(FONT_FAMILY, '', 11)
            self.multi_cell(0, 5, f"{edu.get('degree', '')} ({edu.get('duration', '')})")
            self.ln(3)
...
@st.cache_data(show_spinner="Generating resume from LinkedIn profile...")
def generate_resume_from_text(_llm, text):
    """Uses LLM to parse text and generate a structured resume in JSON."""
    parser = JsonOutputParser()
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an expert resume writer. Your task is to analyze the provided text from a LinkedIn profile and extract the user's information into a structured JSON format. 
        ...
        ("user", "Here is the text from the LinkedIn profile:\n\n{linkedin_text}"),
        ("system", "Please provide the output in JSON format only.")
    ])
    
    chain = prompt | _llm | parser
    response = chain.invoke({"linkedin_text": text})
    return response
...
st.write("Generate a professional resume from your LinkedIn profile and tailor it to a specific job in seconds.")
...
with st.sidebar:
    ...
    st.subheader("1. Upload LinkedIn Profile")
    st.info("Go to your LinkedIn profile > More > Save to PDF. Then upload the file below.")
    linkedin_pdf = st.file_uploader("Upload your LinkedIn PDF", type="pdf")
...
if generate_button:
    ...
    elif not linkedin_pdf:
        st.error("Please upload your LinkedIn profile PDF.")
    else:
        llm = ChatGroq(temperature=0.2, groq_api_key=groq_api_key, model_name="llama3-70b-8192")
        
        # 1. Extract text from PDF
        linkedin_text = extract_text_from_pdf(linkedin_pdf)
        
        if linkedin_text:
            # 2. Generate base resume
            resume_json = generate_resume_from_text(llm, linkedin_text)
